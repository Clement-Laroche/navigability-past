...
### empirical points
comp_points <- st_read("empirical_endpoints/Endpoints_empirical.shp")
...
conf_points <- st_read("partsection_riverpoints/MinervaRiv.shp")
...
ras = rast("raster/SRTM romanFull.tif")
elevation <- extract(x = ras,y = st_coordinates(conf_points)[,1:2])
...
elevation <- extract(x = ras,y = st_coordinates(points)[,1:2])
...
rm(elevation,ras,rivers,i,n,pos,res,my.lm,res_p,seg.lm1,seg.lm2,tab,distance,res_low,res_up)
# creating a list that will hold several information on each river
L<-as.list(rep(NA,n_rivers))
# for each river
for(i in 1:n_rivers)
{
  # we store 14 information on each river
...
pdf(file = "results/fullsection_w_constraint.pdf",onefile = TRUE,paper = "a4")
for(i in 1:n_rivers)
{
  print(L[[i]][[12]])
}
...
save(RES,L,table_points,comp_points,conf_points,file = "results/fullsection_w_constraint.Rdata")
write.table(x = RES,file = "results/fullsection_w_constraint.csv",sep = ";")
...
# saving the results
save(tab_indic,file = "results/Indic_fullsection_w_constraint.Rdata")
